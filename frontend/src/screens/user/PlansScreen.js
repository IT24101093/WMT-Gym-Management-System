...
import { View, Text, TouchableOpacity, ScrollView, ActivityIndicator, Modal, RefreshControl, Image, TextInput, KeyboardAvoidingView, Platform } from 'react-native';
...
export default function PlansScreen({ navigation }) {
  ...
  const [goalForm, setGoalForm] = useState({ targetWeight: '', weeklyFrequency: '', healthNotes: '' });
  ...
  const fetchData = async () => {
    try {
      ...
      setCurrentUser(userProfileRes.data);
      if (userProfileRes.data.gymGoals) {
        setGoalForm({
          targetWeight: String(userProfileRes.data.gymGoals.targetWeight || ''),
          weeklyFrequency: String(userProfileRes.data.gymGoals.weeklyFrequency || ''),
          healthNotes: userProfileRes.data.gymGoals.healthNotes || '',
        });
      }
    } catch (error) {
 ...
 // 🚀 Step 2: Submit the Form Data to Backend (Using Native Fetch!)
  const submitEnrollment = async () => {
    ...
    try {
      ...
      // Show success with onClose callback to navigate to Dashboard
      // This delays navigation until AFTER the user acknowledges, preventing navigation context errors for new users
      showFeedback(
        "Request Sent",
        "You have successfully requested to join! Waiting for Admin approval.",
        "success",
        () => {
          // Navigate back to Dashboard — useFocusEffect will refresh enrollment on screen focus
          navigation.goBack();
        }
      ); 
    } catch (error) {
      ...
      setSelectedPlan(null);
      setActionLoading(null);
      showFeedback("Enrollment Failed", error.message || "An unexpected error occurred", "error");
    }
  };

  const submitGoals = async () => {
    setActionLoading('saving-goals');
    try {
      const payload = {
        gymGoals: {
          targetWeight: Number(goalForm.targetWeight),
          weeklyFrequency: Number(goalForm.weeklyFrequency),
          healthNotes: goalForm.healthNotes,
        }
      };
      const res = await api.put('/users/profile/selections', payload);
      setCurrentUser(res.data);
      setGoalModalVisible(false);
      showFeedback("Goals Updated", "Your gym data has been saved successfully!", "success");
    } catch (error) {
      showFeedback("Update Failed", error.response?.data?.message || "Could not save your goals.");
    } finally {
      setActionLoading(null);
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-black">
      ...
      >
        ...
        <View className="flex-row justify-between items-center mb-4">
          <Text className="text-gray-500 font-bold uppercase text-xs tracking-widest">My Gym Data</Text>
          <TouchableOpacity onPress={() => setGoalModalVisible(true)}>
            <Text className="text-green-500 font-bold text-xs uppercase">Edit</Text>
          </TouchableOpacity>
        </View>
        
        {currentUser?.gymGoals?.targetWeight ? (
          <View className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 mb-8 flex-row justify-around">
            <View className="items-center">
              <Text className="text-gray-500 font-bold text-[10px] uppercase">Goal</Text>
              <Text className="dark:text-white font-black text-lg">{currentUser.gymGoals.targetWeight} kg</Text>
            </View>
            <View className="w-[1px] h-full bg-gray-200 dark:bg-gray-800" />
            <View className="items-center">
              <Text className="text-gray-500 font-bold text-[10px] uppercase">Sessions/Wk</Text>
              <Text className="dark:text-white font-black text-lg">{currentUser.gymGoals.weeklyFrequency}</Text>
            </View>
          </View>
        ) : (
          <TouchableOpacity 
            onPress={() => setGoalModalVisible(true)}
            className="bg-slate-50 dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-700 rounded-3xl p-6 mb-8 items-center"
          >
            <Ionicons name="stats-chart" size={24} color="#64748b" />
            <Text className="text-slate-500 font-bold mt-2">Add your health goals</Text>
          </TouchableOpacity>
        )}
          ...
          <View className="flex-1 bg-black/80 justify-end">
            <View className="bg-white dark:bg-gray-900 w-full p-6 rounded-t-[40px] border-t border-gray-200 dark:border-gray-800 shadow-2xl">
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
                <View className="items-center mb-6">
                  <View className="w-16 h-1 bg-gray-300 dark:bg-gray-700 rounded-full mb-4" />
                  <Text className="text-2xl font-black dark:text-white uppercase italic text-center">Gym Profile Data</Text>
                </View>

                <View className="mb-4">
                  <Text className="text-gray-500 font-bold uppercase text-[10px] tracking-widest ml-2 mb-2">Target Weight (kg)</Text>
                  <TextInput value={goalForm.targetWeight} onChangeText={t => setGoalForm({ ...goalForm, targetWeight: t })} 
                    placeholder="e.g. 75" keyboardType="numeric" placeholderTextColor="#64748b"
                    className="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white p-4 rounded-xl font-bold border border-slate-100 dark:border-slate-800" />
                </View>

                <View className="mb-4">
                  <Text className="text-gray-500 font-bold uppercase text-[10px] tracking-widest ml-2 mb-2">Weekly Frequency</Text>
                  <TextInput value={goalForm.weeklyFrequency} onChangeText={t => setGoalForm({ ...goalForm, weeklyFrequency: t })} 
                    placeholder="How many days per week?" keyboardType="numeric" placeholderTextColor="#64748b"
                    className="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white p-4 rounded-xl font-bold border border-slate-100 dark:border-slate-800" />
                </View>

                <View className="mb-6">
                  <Text className="text-gray-500 font-bold uppercase text-[10px] tracking-widest ml-2 mb-2">Health Notes / Goals</Text>
                  <TextInput value={goalForm.healthNotes} onChangeText={t => setGoalForm({ ...goalForm, healthNotes: t })} 
                    placeholder="Any injuries or specific targets?" multiline placeholderTextColor="#64748b"
                    className="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white p-4 rounded-xl font-bold border border-slate-100 dark:border-slate-800" />
                </View>

                <TouchableOpacity 
                  onPress={submitGoals}
                  disabled={actionLoading === 'saving-goals'}
                  className="w-full bg-green-500 py-4 rounded-2xl items-center shadow-lg mb-3"
                >
                  {actionLoading === 'saving-goals' ? <ActivityIndicator color="black" /> : <Text className="text-black font-black uppercase">Save Data</Text>}
                </TouchableOpacity>

                <TouchableOpacity onPress={() => setGoalModalVisible(false)} className="w-full py-2 items-center">
                  <Text className="text-red-500 font-bold uppercase">Cancel</Text>
                </TouchableOpacity>
              </ScrollView>
            </View>
      ...
      {/* 🔴 CUSTOM FEEDBACK MODAL (Imported correctly) */}
