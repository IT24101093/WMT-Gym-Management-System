...
export default function ManageTrainersScreen({ navigation }) {
  ...
  const openEditModal = (trainer) => {
    ...
    setForm({
      ...
      contact: trainer.contact || "", // 👈 පවතින අංකය ලබා ගැනීම
  ...
  const openAddModal = () => {
    ...
    setForm({
      ...
      age: "",
      specialization: "",
      contact: "", // 👈 Reset කරන විට හිස් කිරීම
      image: null,
      availableFrom: "06:00 AM",
      availableTo: "08:00 PM",
    });
  ...
  const handleSaveTrainer = async () => {
    ...
    try {
      const payload = {
        ...
        contact: form.contact.trim(), // 👈 Backend එකට යැවීම
        image: form.image,
          ...
          <View className="flex-1 bg-black/60 justify-end">
            <View className="bg-white dark:bg-slate-950 p-6 rounded-t-[40px]">
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {/* 📞 අලුතින් එක් කළ WhatsApp Field එක */}
                <Field
